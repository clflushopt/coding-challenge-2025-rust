...
pub fn build_idx(parameter: &Parameter, data: &[i32]) -> Box<[u8]> {
    ...
    if distinct_count <= 150 {
        return build_exact_index(&freq_map, min_val, max_val);
    }
    ...
    let bloom_size = calculate_bloom_size(ratio, distinct_count, data.len());
...
fn build_exact_index(freq_map: &HashMap<i32, u64>, min_val: i32, max_val: i32) -> Box<[u8]> {
    let num_entries = freq_map.len() as u32;
    let size = 1 + 8 + 4 + (num_entries as usize * 12);
