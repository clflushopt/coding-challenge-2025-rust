...
pub fn build_idx(parameter: &Parameter, data: &[i32]) -> Box<[u8]> {
    ...
    if unique_count <= 500 && ratio > 0.5 {
        return build_hashmap_index(&counts);
    }

    if unique_count <= 1000 && ratio > hashmap_size_kb * 0.7 {
        return build_hashmap_index(&counts);
    }

    if range < 100000 && bitmap_size_kb < 15.0 && ratio > 0.8 {
        if bitmap_size_kb < hashmap_size_kb * 0.6 {
            return build_bitmap_index(min_val, max_val, &counts);
        }
    }

    if unique_count <= 2000 && ratio > hashmap_size_kb * 1.2 {
        return build_hashmap_index(&counts);
    }

    if unique_count <= 5000 && ratio > hashmap_size_kb * 2.5 {
        return build_hashmap_index(&counts);
    }
