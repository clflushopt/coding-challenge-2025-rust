...
pub fn build_idx(parameter: &Parameter, data: &[i32]) -> Box<[u8]> {
    ...
    let range = (max_val as i64 - min_val as i64 + 1) as f64;
    let density = unique_count as f64 / range;

    let hashmap_size_bytes = 5 + unique_count * 12;
    let hashmap_size_kb = hashmap_size_bytes as f64 / 1024.0;


    if unique_count <= 50 && ratio > 0.2 {
        return build_hashmap_index(&counts);
    }

    if unique_count <= 200 && ratio > 0.4 {
        return build_hashmap_index(&counts);
    }

    if unique_count <= 500 && ratio > hashmap_size_kb * 0.4 {
        return build_hashmap_index(&counts);
    }

    if density < 0.01 && unique_count <= 1000 && ratio > hashmap_size_kb * 0.5 {
        return build_hashmap_index(&counts);
    }

    if unique_count <= 1000 && ratio > hashmap_size_kb * 0.8 {
        return build_hashmap_index(&counts);
    }

    if unique_count <= 2000 && ratio > hashmap_size_kb * 1.5 {
        return build_hashmap_index(&counts);
    }

    if unique_count <= 5000 && ratio > hashmap_size_kb * 3.0 {
        return build_hashmap_index(&counts);
    }
