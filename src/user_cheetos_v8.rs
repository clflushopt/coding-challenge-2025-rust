...
const INDEX_ULTRA_COMPRESSED: u8 = 4;
...
pub fn build_idx(parameter: &Parameter, data: &[i32]) -> Box<[u8]> {
    ...
    let top_20_count = distinct_count.min(20);
    let top_20_coverage: u64 = sorted_freq.iter().take(top_20_count).map(|(_, c)| c).sum();
    let concentration = top_20_coverage as f64 / total_elements as f64;

    if ratio > 35.0 && concentration > 0.6 && distinct_count <= 50 {
        return build_ultra_compressed(&freq_map, min_val, max_val);
    }

    let range_u64 = (max_val as i64 - min_val as i64 + 1) as u64;
    if range_u64 <= 262144 && distinct_count as f64 / range_u64 as f64 > 0.12 {
        let bitmap_size = (range_u64 as usize + 7) / 8;
        if bitmap_size < 28672 && ratio > 4.0 {
            return build_dense_bitmap(&freq_map, min_val, max_val, range_u64 as usize);
        }
    }

    if distinct_count <= 180 {
        return build_exact_compressed(&freq_map, min_val, max_val);
    }

    if distinct_count <= 500 && ratio > 12.0 {
        return build_exact_compressed(&freq_map, min_val, max_val);
    }

    if distinct_count <= 800 && ratio > 25.0 {
        return build_exact_compressed(&freq_map, min_val, max_val);
    }

    if distinct_count <= 1500 && ratio > 40.0 && concentration > 0.4 {
        return build_exact_compressed(&freq_map, min_val, max_val);
    }

    let top_k = calculate_adaptive_top_k(ratio, distinct_count, concentration);
    let bloom_size = calculate_adaptive_bloom_size(ratio, distinct_count, concentration);
...
fn calculate_adaptive_top_k(ratio: f64, distinct_count: usize, concentration: f64) -> usize {
    ...
    base_k.min(distinct_count / 2).max(10)
}

fn calculate_adaptive_bloom_size(ratio: f64, distinct_count: usize, concentration: f64) -> usize {
    let base_fpr = if concentration > 0.5 {
        0.020
    ...
    let (min_size, max_size) = if ratio > 25.0 {
        (1024, 7168)
    } else if ratio > 15.0 {
        (768, 5120)
    } else if ratio > 8.0 {
...
fn build_ultra_compressed(freq_map: &HashMap<i32, u64>, min_val: i32, max_val: i32) -> Box<[u8]> {
    let mut entries: Vec<_> = freq_map.iter().map(|(&k, &v)| (k, v.min(65535))).collect();
    entries.sort_unstable_by_key(|&(k, _)| k);

    let mut buffer = Vec::new();
    buffer.push(INDEX_ULTRA_COMPRESSED);
...
fn build_hybrid_index(
    ...
    let num_hashes = optimal_k.clamp(6, 10);
...
pub fn query_idx(parameter: &Parameter, index: &[u8], query: &i32) -> Option<u64> {
    ...
    match index[0] {
        ...
        INDEX_ULTRA_COMPRESSED => {
            ...
            let num_entries = index[9] as usize;
            let mut offset = 10;

            for _ in 0..num_entries {
                if offset + 6 > index.len() {
                    return None;
                }
                ...
                let count = u16::from_le_bytes([index[offset + 4], index[offset + 5]]) as u64;
                offset += 6;
